...
function getTeams(callback){
	...
	prepareDb(function(conn){
		conn.query("SELECT uid,name FROM ffgame_wc.master_team ORDER BY name LIMIT 100;",
			[],function(err,team){
				conn.release();
				callback(err,team);
				...
				function(players,callback){
					...
					async.eachSeries(players,function(player,next){
						...
						conn.query("SELECT SUM(total_points) AS points,\
									...
									FROM ffgame_stats_wc.master_player_performance \
									WHERE player_id = ?)\
									UNION ALL\
		...
		async.waterfall([
				function(callback){
					conn.query("SELECT a.uid AS player_id,a.team_id,\
								...
								INNER JOIN ffgame_stats_wc.master_match_player_points b\
...
function getTeamById(team_uid,callback){
	prepareDb(function(conn){
		conn.query("SELECT uid,name FROM ffgame_wc.master_team WHERE uid = ? LIMIT 1;",
...
function create(data,callback){
	prepareDb(function(conn){
			...
			[
				function(callback){
					conn.query("SELECT id FROM ffgame_wc.game_users WHERE fb_id=? LIMIT 1",
								[data.fb_id],
								function(err,rs){
									//console.log(this.sql);
									callback(null,rs[0]);
				...
				function(user,callback){
					if(user==null){
						...
						console.log(user);
						conn.query("INSERT INTO ffgame_wc.game_teams\
				...
				function(result,callback){
					if(result!=null){
						...
						var sql = "INSERT IGNORE INTO ffgame_wc.game_team_players\
...
function remove_team(game_team_id,callback){
	prepareDb(function(conn){
		async.waterfall(
			[
				function(callback){
					conn.query("DELETE FROM ffgame_wc.game_teams WHERE id=? LIMIT 1",
								[game_team_id],
								function(err,rs){
									callback(err);
								});
				...
				function(callback){
					conn.query("DELETE FROM ffgame_wc.game_team_players WHERE game_team_id=?",
								[game_team_id],
								function(err,rs){
									callback(err,'');
								});
			...
			[
				...
				function(user,callback){
					if(typeof user !=='undefined'){
						conn.query("SELECT * FROM ffgame_wc.game_teams WHERE user_id = ? LIMIT 1",[
							user.id
						],
							function(err,team){
								//console.log(team);
