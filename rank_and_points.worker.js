		...
		if(resp.status==1){
			...
			pool.getConnection(function(err,conn){
				process_report(
					...
					conn.query("UPDATE ffgame_stats_wc.job_queue_rank \
								SET finished_dt = NOW(),\
									n_status = 2 \
								WHERE id = ?",
		...
		async.waterfall([
			function(cb){
				conn.query("SELECT matchday \
							FROM \
							ffgame_stats_wc.game_team_player_weekly \
			...
			function(game_id,matchday,cb){
				conn.query("SELECT * FROM ffgame_wc.game_perks \
					WHERE game_team_id=? \
					AND matchday=? \
					AND n_status=0 \
					LIMIT 100;",
...
function flagPerkAsDone(conn,perk_id,done){
	conn.query("UPDATE ffgame_wc.game_perks SET n_status=1 WHERE id = ?",
