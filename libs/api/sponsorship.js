...
function getAvailableSponsorship(game_team_id,callback){
	prepareDb(function(conn){
		conn.query("SELECT a.id,name,value,expiry_time,is_available \
				FROM "+config.database.database+".game_sponsorships a\
				WHERE a.is_available = 1 AND NOT EXISTS(\
					SELECT 1 FROM "+config.database.database+".game_team_sponsors b\
	...
	prepareDb(function(conn){
		async.waterfall([
			function(callback){
				...
				//detil sponsorship
				conn.query("SELECT * FROM "+config.database.database+".game_sponsorships WHERE id=? LIMIT 1;",
							[sponsor_id],
							function(err,rs){
								try{
									callback(err,rs[0]);
			...
			function(sponsor_data,callback){
				...
				conn.query(
					"SELECT a.* FROM "+config.database.database+".game_sponsor_perks a\
			...
			function(sponsor_data,callback){
				conn.query("SELECT * FROM "+config.database.database+".game_team_sponsors\
			...
			function(sponsor_data,has_sponsor,callback){
				...
				if(!has_sponsor){
					conn.query("INSERT INTO "+config.database.database+".game_team_sponsors\
...
function getActiveSponsors(game_team_id,callback){
	prepareDb(function(conn){
		conn.query("SELECT b.name,b.value,a.valid_for \
					FROM "+config.database.database+".game_team_sponsors a\
